...
fun ScreenAddName(
    ...
    Scaffold(
        ...
        Column(
            ...
            TextFieldForAuth(hint = "Имя (обязательно)", user = com.turitsynanton.android.wbtech.data.storage.models.DataUser()) {
                name = it.name
                buttonEnable = name.length > 1
            }
            Spacer(modifier = Modifier.padding(bottom = 12.dp))
            TextFieldForAuth(hint = "Фамилия (опционально)", user = com.turitsynanton.android.wbtech.data.storage.models.DataUser()) {
                surname = it.surname
            }
